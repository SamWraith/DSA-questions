    ...
    long long continuousSubarrays(vector<int>& nums) {
        ...
        for (int r = 1; r < nums.size(); r++) {
            if (left <= nums[r] && nums[r] <= right) {
                ...
                right = nums[r] + 2;
                l = r;
                // If we consistently move leftward in each iteration, it
                // implies that the entire left subarray satisfies the given
                // condition. For every subarray with l in the range [0, r], the
                // condition is met, preventing the code from reaching the final
                // "else" condition. Instead, it stops at the "if" condition.
